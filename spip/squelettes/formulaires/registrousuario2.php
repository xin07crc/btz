 ...
 * @return array
 */
function formulaires_registrousuario2_charger_dist($idioma, $ok, $err1, $err2, $err3, $err4, $err5, $error6, $inv){
    $valeurs = array();
 ...
 * @return array
 */
function formulaires_registrousuario2_verifier_dist($idioma, $ok, $err1, $err2, $err3, $err4, $err5, $error6, $inv){
    session_start();
    ...
    $select_telefono = $conn->prepare("SELECT * from registroreto WHERE telefono = :telefono");
 ...
 *
 */
function formulaires_registrousuario2_traiter_dist($idioma, $ok, $err1, $err2, $err3, $err4, $err5, $error6, $inv){
    $conn =conectar();
    ...
    $insert = $conn->prepare("INSERT INTO registroreto (nombre, apellido, email, fecha, token, token_url, ip_registro, telefono) VALUES(:nombre, :apellidos, :email, :fecha, :token, :token_url, :ip_registro, :telefono)");
    $valores = array(
        ":nombre" => $nombre,
        ":apellidos" => $apellido,
        ":email" => $correo,
    ...
    if ($insert->execute($valores)) {
        if ($insert->rowCount() > 0) {
            ...
            if ($_SERVER["SERVER_NAME"] == "localhost") {
                echo 'http://localhost/typ/spip/validar2/validar.php?token=' . $token . "|" . $token_url . "&lang=" . $idioma;
                $mail = true;
            }
            else {
