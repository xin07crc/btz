 ...
 * @return array
 */
function formulaires_loginusuariosmsmanaguer_charger_dist($idioma, $ok, $err1, $err2, $err3, $err4){
    $valeurs = array();
 ...
 * @return array
 */
function formulaires_loginusuariosmsmanaguer_verifier_dist($idioma, $ok, $err1, $err2, $err3, $err4){
    $erreurs = array();
    $texto_error = "";
    $conn =conectar();
 ...
 *
 */
function formulaires_loginusuariosmsmanaguer_traiter_dist($idioma, $ok, $err1, $err2, $err3, $err4){
    $conn = conectar();
    ...
    if (is_numeric($correo)) {
        $select = $conn->prepare("SELECT * FROM btz_managuer WHERE telefono = :telefono");
        if ($select->execute(array(":telefono" => $correo))) {
            if ($select->rowCount() > 0) {
                ...
                $comprobar_sms = comprobar_sms_betatester($conn, $id_usuario, ESPERA);
                ...
                if ($comprobar_sms["enviado"] == false) {
                    ...
                    if ($enviar_sms["error"] == false) {
                        ...
                        $almaceno_sms = almaceno_sms_manager($conn, $codigo, $id_usuario);
        ...
        if ($select->execute(array(":email" => $correo))) {
            if ($select->rowCount() > 0) {
                ...
                if ($_SERVER["SERVER_NAME"] == "localhost") {
                    echo "http://localhost/typ/spip/?page=btz_&token=" . $token . "&fl=" . $fecha_mili;
                    $mail = true;
                } else {
                    //$mail = true;
...
function crearTokenFecha($conn, $date, $debug) {
    ...
    $select = $conn->prepare("SELECT * FROM btz_managuer WHERE token = :token");
