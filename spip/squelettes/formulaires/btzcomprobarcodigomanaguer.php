...
function formulaires_btzcomprobarcodigomanaguer_charger_dist($token, $token_url) {
    $valeurs = array();
...
function formulaires_btzcomprobarcodigomanaguer_verifier_dist($token, $token_url) {
    $texto_error = "";
    if (!$_POST["codigo"]) {
...
function formulaires_btzcomprobarcodigomanaguer_traiter_dist($token, $token_url) {
    session_start();
    $conn = conectar();
    // Obtengo los datos del usuario
    $datos_usu = comprobar_usuario_managuer($conn, $token);
    ...
    $select = $conn->prepare("SELECT * FROM btz_enviosms WHERE id_managuer = :id_usuario ORDER BY fecha DESC limit 1");
    ...
    if ($select->execute(array(":id_usuario" => $id_usuario))) {
        ...
        if ($select->rowCount() > 0) {
            ...
            if ($codigo_bbdd == $codigo) {
                $update = $conn->prepare("UPDATE btz_managuer SET status = 1 WHERE id_usuario = :id_usuario");
                if ($update->execute(array(":id_usuario" => $id_usuario))) {
                    ...
                    return array(
                        'message_ok' => 'User activated, go to <a href="?page=btz_manager&token=' . $token . '&fl=' . $fecha_mili . '">Betatester</a>'
                    );
                }
            }
            else {
