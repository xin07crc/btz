...
if (isset($_GET["token"])) {
    ...
    $usuario = comprobar_usuario_tokens($conn, $t_usu, $t_url);
    if ($usuario["error"] == false) {
        // Compruebo si se ha enviado el SMS al usuario
        $comprobar_sms = comprobar_sms($conn, $usuario["id_usuario"], ESPERA);
        ...
        if ($comprobar_sms["enviado"] == false) {
            ...
            if ($enviar_sms["error"] == false) {
                ...
                $almaceno_sms = almaceno_sms($conn, $codigo, $usuario["id_usuario"]);
